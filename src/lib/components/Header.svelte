<script lang="ts">
	...
	import Wordmark from './icons/Wordmark.svelte';
...
<!-- ╔══════════════════════════════════════════════════════════════════╗
     ...
     ╚══════════════════════════════════════════════════════════════════╝ -->
<header
	class="sticky top-0 z-[100] flex h-20 w-screen items-center justify-center bg-[var(--primary-green)] p-4 backdrop-blur-md"
>
	<nav class="flex h-full w-full max-w-screen-lg">
		...
		<div class="flex items-center justify-end">
			<div class="relative pr-1">
				...
				{#if $cartCount > 0}
					<span
						class="text-md absolute right-0.5 -bottom-1 flex h-5 w-5 items-center justify-center rounded-full bg-[var(--light-green)] p-1 leading-none font-semibold text-white ring-2 ring-white"
...
{#if cartOpen}
	...
	<div class="pointer-events-none fixed inset-x-0 top-20 z-[95] flex justify-center">
		...
		>
			<div
				class="rounded-none border border-zinc-200 bg-white shadow-xl ring-1 ring-black/5 backdrop-blur-xl"
			>
				<!-- DRAWER HEADER -->
				...
				{:else}
					<ul class="max-h-[45vh] overflow-auto px-2 py-2 sm:px-0">
						{#each $cart.lines as l}
							<li
								class="mb-2 flex items-center justify-between gap-4 rounded-xl border border-neutral-200/70 bg-white px-4 py-3"
							>
								<div class="min-w-0">
					...
					<!-- DRAWER FOOTER -->
					<div
						class="flex items-center justify-between border-t border-neutral-200 px-4 py-4 sm:px-6"
					>
						<div class="text-sm text-neutral-700">Subtotal</div>
						<div class="text-base font-semibold text-neutral-900 tabular-nums">
